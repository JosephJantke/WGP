...
directory = "E:/SZ673_BlackchinnedHoneyeater/jj81-90"  # todo change this when I move on to the next directory
os.chdir(directory)
home = directory
...
for file in glob.glob(folder, recursive=True):
    ...
    second_line_fragmented[4] = str(4.992)
    second_line_fragmented[8] = str(4.992)
    ...
    third_line_fragmented[3] = str(5.088)  #str() is necessary to turn floats back into strings for .join()
    third_line_fragmented[4] = str(10.088)
    third_line_fragmented[6] = '16000'
    third_line_fragmented[8] = str(5.0)
    ...
    ### write the rest of the entries - alternating in start, end, and duration
        for line in content_odd[1:]:
            ...
            lf[3] = (lf[0]-1)/2 * 10.184  # enter correct values
            lf[4] = lf[3] + 4.992
            ...
            lf[8] = str(4.992)  # convert duration to correct value
            ...
            lf[6] = '16000'
            line_no = int(lf[0])  # create this variable for writing even entries in below
            lf[3] = 15.272 + (((lf[0])/2 - 2) * 10.184)  # enter correct values
            lf[4] = lf[3] + 5.00
            lf[3] = round(lf[3], 3)  # use round() to prevent long strings of decimal places
            lf[4] = round(lf[4], 3)
            lf[3] = str(lf[3])  # convert floats to string to be able to rejoin the line into a single string
